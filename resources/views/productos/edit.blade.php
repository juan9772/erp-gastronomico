                ...
                <form method="POST" action="{{ route('productos.update', $producto) }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="precio_venta" name="precio_venta" type="number" step="0.01" class="mt-1 block w-full" :value="old('precio_venta', $producto->precio_venta)" required />
                        ...
                        <div>
                            ...
                            <select id="receta_id" name="receta_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach($recetas as $receta)
                                    <option value="{{ $receta->id }}" @if(old('receta_id', $producto->receta_id) == $receta->id) selected @endif>{{ $receta->nombre }} (Costo: ${{ number_format($receta->costo_total, 2) }})</option>
                                @endforeach
                            </select>
